...
const Banker = () => {
  ...
  const [bankers, setBankers] = useState<Banker[]>(initialData);
  ...
  const handleDelete = (id: number) => {
    if (window.confirm("Are you sure you want to delete this banker?")) {
      setBankers(bankers.filter((item) => item.id !== id));
    }
  };

  const handleBulkDelete = () => {
    if (window.confirm("Are you sure you want to delete selected bankers?")) {
      setBankers(bankers.filter((item) => !selectedIds.includes(item.id)));
      setSelectedIds([]);
    }
  };
  ...
  const onFormSubmit = (data: FormValues) => {
    if (editId) {
      ...
      // Add new
      const newId = Math.max(...bankers.map((item) => item.id)) + 1;
      setBankers([
        ...bankers,
        {
          ...data,
